...
Successful PANEL dataset responses are cached as read-only `panel_snapshots` keyed by owner, contract, AppView, dataset, normalized filters, search, page, pageSize, and `configRevision`. On a network failure the Client may show a compatible snapshot with an offline notice. Snapshots from another contract, AppView, dataset, filter set, page, page size, search, or known different config revision are not used. PANEL never creates `pending_operations`, never reuses `STATE_UPDATE`, and does not reuse REPORT snapshots as a parallel source. The Client performs no formulas, metrics, SQL, JavaScript, aggregations, or business calculations for PANEL.
