...
| Attendance adapter | Attendance UX mapped to `STATE_UPDATE`. | `AttendanceWorkflow.tsx`, `attendance-offline.ts`, `attendance-workflow-logic.ts`. | Person, date, status option, optional observation, configured `contextFieldIds`. | State-update intent and Attendance-shaped UI. | Component state; last context option per owner/contract/AppView/fieldId in `app_metadata`; shared state-update storage. | State-update runtime, source Personas cache. | Operational Core online. | CONFLICT, NETWORK, SQLITE, missing prewarm, invalid remembered option. |
...
`REPORT` is a configurable consultation renderer, separate from RECORDS, BOARD, DASHBOARD, PANEL, and workflows. It calls `/api/v1/contracts/:contractId/reports/:appViewId` and never creates, updates, or syncs records. `TABLE` and `MATRIX` use one shared `from/to` period: `timeFilter.mode = RANGE` shows Desde/Hasta inputs; `MONTH` shows a compact month navigator and converts the selected month to first/last day. Missing `timeFilter` falls back to editable current-month RANGE. `TABLE` renders one record per row using configured columns. `MATRIX` groups rows by `rowFieldId`, generates columns from `columnFieldId`, renders cells from `valueFieldId`, and optionally counts `summaryFieldId` values per row. `LATEST_BY_RELATION` renders a derived latest-row list from the report payload: one row per related record, with dynamic columns ordered by `latestByRelation.displayFieldIds`; the compatible `CURRENT_STATUS` alias still reads legacy `currentStatus`. The renderer uses the configured field names as headings, never infers singulars by removing suffixes, has search by name, does not show period navigation, and does not substitute missing order/date fields with `createdAt`, `updatedAt`, or audit metadata. Successful report responses are cached as read-only snapshots for offline fallback. For SELECT and MULTISELECT report fields, `config.valueDisplay[fieldId]` chooses visible labels or internal values; missing entries default to labels, and internal mode uppercases the internal value for presentation while falling back to the label when no internal value exists. For monthly date matrices, the renderer generates the real days in the selected month. For a monthly Attendance view this can be configured as Persona x Fecha with Estado as both value and summary, but the renderer does not hardcode Attendance statuses or field names.

`PANEL` is the modular future surface for composed experiences. The Client consumes `/api/v1/contracts/:contractId/panels/:appViewId` and treats Core as the execution boundary: datasets, filters, search, pagination, latest-by-relation reduction, and future calculations run server-side. The renderer supports `TABLE` and `KPI` modules, loads each required `datasetId` once per query state even when multiple modules share it, keeps pagination state per dataset, and isolates dataset errors so one failed module does not crash modules backed by other datasets. TABLE columns come exactly from `visualization.config.columns`; the Client does not prepend relation fields, infer columns from dataset `fieldIds`, reorder, or duplicate columns. Field ids remain semantic identity; names are display metadata. Unsupported module visualization types render a controlled generic state.

On desktop, PANEL rendering is coordinate-based: every module uses the persisted `layout.x`, `layout.y`, `layout.w`, and `layout.h` against `layout.columns` and `layout.rowHeight`. Empty grid cells remain empty, module widths/heights are not recalculated from array order, and the Client does not compact or rerun the editor's automatic organization while rendering. `layout.distribution = AUTO`, `MANUAL`, and legacy configs without `distribution` all render the coordinates received from Core or cache; AUTO is an editor behavior, not a Client renderer instruction. Compact `rowHeight` values from Core are logical grid units, not a direct instruction to shrink KPI/table content to a few pixels; the renderer resolves an effective visual row height that preserves the previous readable KPI/table presentation before placing the next logical row. KPI value text keeps its established size and weight, declares an explicit line height for numeric glyphs, and prevents the value line from shrinking away when tablet widths make labels or updated-at metadata wrap. Because long KPI labels and dates can require more vertical space than the generic module minimum, KPI modules reserve a larger visual minimum and the same effective row height is applied to `top` and `height`, so following rows move down instead of overlapping. This does not change saved coordinates or module distribution. On mobile, modules stack in stable spatial order (`y`, then `x`, then id) at full width so the screen does not introduce global horizontal overflow, and this stacked presentation never mutates the stored `PanelConfig`. The verified Client-side cause of OPCL-PANEL-010 was the previous use of sorted modules plus flex wrapping: `x` only influenced order, so a module persisted at `x=9,y=2` could wrap to the start of the next visual row. The follow-up OPCL-PANEL-010C fixed the regression introduced by treating `rowHeight = 6` as six rendered pixels, which made modules overlap even though their saved coordinates were correct. OPCL-KPI-013 confirmed that `lineHeight: 42` was active but only addressed glyph clipping; the tablet disappearance was vertical flex compression inside two-row KPI modules. Core editor summary cards are a separate configuration UI surface and must not be mirrored into this runtime PANEL renderer.

Successful PANEL dataset responses are cached as read-only `panel_snapshots` keyed by owner, contract, AppView, dataset, normalized filters, search, page, pageSize, and `configRevision`. On a network failure the Client may show a compatible snapshot with an offline notice. Snapshots from another contract, AppView, dataset, filter set, page, page size, search, or known different config revision are not used. A composed KPI consumes its `moduleResults` value from one PANEL response, including when offline; Client never combines metrics from separate snapshots. The assigned PANEL AppView supplies `configRevision` before the first dataset request, and composed KPIs require that revision for offline fallback. It shows Core's unavailable reason, and formats `PERCENT` ratios once (`0.75` becomes `75%`). Old KPI modules continue to use `metricId`. A changed filter query hides the previous KPI value while its new response is pending; late responses do not replace the current query. PANEL never creates `pending_operations`, never reuses `STATE_UPDATE`, and does not reuse REPORT snapshots as a parallel source. The Client performs no formulas, metrics, SQL, JavaScript, aggregations, or business calculations for PANEL.
...
Experience opening performance uses one shared route session and one measurement id per opening. It covers RECORDS, PANEL, REPORT, attendance, generic state update, and controlled unsupported renderers. Direct entry and reload begin at route entry; the current navigation cards do not expose a durable pre-route event, so the measurement explicitly says `route` and must not be interpreted as time since click. Legacy RECORDS entries remain readable and identify their renderer-start limitation. Renderers publish concrete milestones: RECORDS preserves cache-first rows and remote completion; workflows become ready only when their controls and required response are available; REPORT treats a valid empty result as useful; PANEL records first content plus settled loaded/empty/failed module counts. Unsupported views terminate as controlled errors rather than successful content.

Local read, remote refresh, module requests, and preparation may run concurrently, so durations are reported independently and are never added as a synthetic total. One measurement id is updated at useful-presentation and terminal milestones; unmount produces `cancelled`, a storage reload converts unfinished persisted entries to `interrupted`, and safe error classification stores only codes. Refreshes and searches update renderer state without creating another opening. History writes are best-effort and serialized per fingerprinted owner/contract scope, so diagnostics cannot delay content or overwrite logout cleanup.

Repeated diagnostic records use one compact disclosure row each. Rows start collapsed, retain independent expansion while their stable id is updated, and remount when owner/contract scope changes. Their details wrap inside the modal's single vertical scroll; internal retry/copy actions are separate from the expansion control. Copy formatters consume the underlying diagnostic model, never the visual expansion state, so collapsed rows do not omit details.
...
| Experience opening diagnostics | Monotonic stage timings from route entry (legacy RECORDS: renderer mount); wall clock only identifies the opening. | `app_metadata`, newest 20 total across types; unfinished reload entries become interrupted. | Fingerprinted owner + contract; cleared for the owner at logout. | Global `Diagnostico > Rendimiento`, compact details and safe full copy. | Yes. |
