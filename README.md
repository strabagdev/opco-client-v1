...
El encabezado centraliza actividad, pendientes y resultados de escritura mediante un indicador informativo no interactivo. Distingue `Guardado localmente` de confirmaciones de Opco, conserva errores/conflictos prioritarios y se centra respecto del encabezado completo o pasa a una segunda fila en anchos compactos. La tabla de prioridades, el ciclo transitorio de 3,5 segundos, el aislamiento por alcance, los avisos retirados/conservados y las decisiones responsive se documentan en [App Shell Status](docs/CLIENT_ARCHITECTURE.md#app-shell-status). El estado vigente y los pendientes para retomar trabajo estan en [docs/STATUS.md](docs/STATUS.md).
...
El modal `Diagnostico` usa el mismo resultado que el encabezado. Su checklist e historial existentes incluyen actividad visible y resultado de escritura sin crear historiales paralelos; la copia mantiene las exclusiones de registros, payloads y tokens. La especificacion vigente esta en [CLIENT_ARCHITECTURE.md](docs/CLIENT_ARCHITECTURE.md#app-shell-status).
