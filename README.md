...
`PANEL` es la superficie modular futura. En esta etapa consume `/api/v1/contracts/:contractId/panels/:appViewId`, ejecuta datasets server-side en Core y renderiza modulos `TABLE` y `KPI`. Cada request ejecuta el dataset seleccionado por `datasetId`; filtros, busqueda y paginacion se envian al endpoint, y si varios modulos usan el mismo dataset el renderer deduplica la carga local. Las columnas TABLE se toman exactamente desde `visualization.config.columns`: no se antepone relacion, no se infiere desde `fieldIds`, no se reordena y no se duplican columnas. En escritorio, Client posiciona cada modulo con las coordenadas persistidas `x/y/w/h`, `columns` y `rowHeight`, conservando huecos y dimensiones tanto en AUTO como en MANUAL; `rowHeight` compacto se trata como unidad logica de grilla y se escala para mantener KPI y TABLE legibles antes de colocar la fila siguiente. Los valores KPI mantienen su tamaño visual, declaran una caja de linea explicita y reservan altura suficiente para titulo/fecha en dos lineas en anchos tablet; si esa altura aumenta, la misma escala vertical mueve las filas siguientes para evitar superposiciones. Organizar es una accion del editor, no del renderer; las tarjetas horizontales del editor Core no cambian esta presentacion runtime. En movil, los modulos se apilan a ancho completo en orden espacial estable sin modificar la configuracion. SELECT/MULTISELECT usan metadata de opciones, DATE se muestra como `DD-MM-YYYY`, DATETIME como fecha/hora local `es-CL`, y RELATION usa el nombre visible. PANEL no calcula formulas ni agregaciones en Client.
...
El encabezado muestra un unico indicador informativo de icono y texto, sin comportamiento de boton, junto al boton existente de `Diagnostico`. El mismo resolver alimenta el encabezado y `Diagnostico > Sincronizacion`: observa actividad viva de la experiencia visible en RECORDS, PANEL, REPORT y los workflows attendance/state-update, incluidos refrescos posteriores, concurrencia y resultados parciales de PANEL. Cada renderer solo reporta sus estados de carga ya existentes; no inicia peticiones ni modifica sincronizacion o cache. Las ejecuciones se aislan por usuario, contrato y experiencia, y una respuesta tardia de una ruta anterior no puede limpiar la actividad actual. Un fallo retenido se mantiene como `Requiere atención` hasta una lectura posterior exitosa o hasta salir de ese alcance.

| Prioridad | Etiqueta del encabezado | Condicion |
| --- | --- | --- |
| 1 | `Requiere atención` | Recuperacion local, error, conflicto o fallo retenido de la experiencia visible. |
| 2 | `Sin conexión` | Navegador offline o conectividad desconocida; agrega la cantidad de cambios pendientes cuando existe. |
| 3 | `Sincronizando cambios…` | Envio real de cambios pendientes activo. |
| 4 | `Comprobando disponibilidad` | Readiness activo de Operational Core. |
| 5 | `Restaurando sesión` | Restauracion de autenticacion activa. |
| 6 | `Actualizando <experiencia>…` | Una o mas lecturas o actualizaciones de la experiencia visible; agrega pendientes fiables si existen. |
| 7 | `N cambios pendientes` | Trabajo durable pendiente sin envio activo. |
| 8 | `Al día` | Online, sin actividad relevante, pendientes ni problemas conocidos. |

`Al día` no garantiza que todas las experiencias esten precargadas ni que datos aun no consultados sean recientes. La preparacion offline general permanece separada y no activa por si sola el indicador. Los avisos genericos duplicados de refresco de RECORDS y las fases generales de sincronizacion en state-update se omiten cuando el encabezado ya comunica ese estado; se conservan carga inicial, vacios, cobertura offline/parcial, errores contextuales, conflictos, confirmaciones y acciones de reintento o recuperacion.
...
Antes de este cambio, la navegacion esperaba AppViews remotas, el renderer esperaba la definicion remota y la carga sin busqueda esperaba todas las paginas remotas mas la reconciliacion antes del primer `setRecords`. El fixture sintetico de 388 registros demuestra cuatro peticiones paginadas secuenciales en esa ruta. `Diagnostico > Rendimiento` conserva un unico historial de las ultimas 20 aperturas del usuario y contrato activos para `RECORDS`, `PANEL`, `REPORT`, `WORKFLOW` (`attendance` y `state-update`) y las vistas controladas no soportadas `BOARD`/`DASHBOARD`. Registra tipo, variante, origen de medicion, fuente inicial, primera presentacion util, disponibilidad para operar y etapas que realmente existen; PANEL agrega modulos cargados, vacios y fallidos. RECORDS antiguo sigue siendo legible y se identifica como iniciado en el renderer; las aperturas nuevas comienzan al entrar a la ruta, no se presentan como tiempo desde un clic que no fue observado. Las etapas concurrentes se informan por separado y no se suman como total.

El historial se guarda en `app_metadata` con scope fingerprinted, marca como interrumpida una apertura inconclusa tras recarga y se elimina para el usuario al cerrar sesion. En el modal, registros repetidos de Rendimiento, checklist, historial y errores aparecen como filas compactas contraidas, expandibles de forma independiente y sin scroll interno; el resumen de Rendimiento prioriza `N ms · primeras filas`, no el refresco remoto. Cambiar de usuario o contrato reinicia el estado visual de expansion. `Copiar diagnostico de rendimiento` incluye el detalle completo aunque las filas esten contraidas y conserva las exclusiones de scopes, busquedas, payloads, valores, correos y tokens. Estos tiempos permiten diagnosticar una demora futura, pero no reconstruyen episodios anteriores ni convierten fixtures sinteticos en mediciones del dispositivo.
...
### Recuperacion y unicidad en RECORDS

Los errores retenidos de RECORDS se cargan directamente desde el store local, aislados por `ownerKey + contractId + entityTypeId`, para que un aviso de una entidad/contrato no aparezca en otra experiencia. El cliente conserva compatibilidad con errores legacy: `INVALID_RELATION` sigue identificando relaciones invalidas antiguas, mientras que `UNIQUE_FIELD_CONFLICT` y `UNIQUE_CONSTRAINT` se tratan como conflictos definitivos de unicidad. Cuando la respuesta HTTP trae detalles estructurados, la UI usa el campo/codigo/mensaje real; cuando solo existe el formato legacy, muestra una explicacion generica sin inventar datos.

La recuperacion de errores reutiliza la operacion pendiente existente. Corregir un `CREATE` o `UPDATE` fallido actualiza el payload pendiente y el snapshot local en una transaccion; no crea una segunda operacion ni pierde el `clientRequestId`. Descartar un `CREATE` solo esta permitido si el registro todavia no tiene identidad remota. Descartar un `UPDATE` requiere recuperar primero la version remota para restaurar el snapshot local antes de eliminar la intencion pendiente. Las acciones estan protegidas contra doble ejecucion con estado local de accion en curso y confirmaciones explicitas en Web y Native.

Los avisos huerfanos solo se cierran cuando ya no queda operacion pendiente ni registro local fallido/conflictivo para el mismo contexto, o cuando una sincronizacion/accion de recuperacion termina con exito verificable. La marca `translate="no"` se usa solo para proteger valores tecnicos, codigos, ids y mensajes de backend que no deben traducirse automaticamente; no convierte toda la pantalla en contenido tecnico. La resolucion real de errores observados en un dispositivo de usuaria debe registrarse solo despues de evidencia en ese dispositivo; el codigo implementa el flujo, pero eso no confirma por si solo que aquellos dos errores historicos hayan cerrado en campo.

La validacion preventiva de unicidad usa `EntityField.isUnique` y es generica: no esta hardcodeada a RUT ni a ningun identificador de dominio. En local compara contra registros cacheados y operaciones pendientes del mismo `ownerKey + contractId + entityTypeId`, excluyendo el propio registro al editar. La cobertura del cache es parcial: si el dispositivo no tiene todos los registros, la comprobacion local puede no ver una colision remota. Cuando hay red y token, el cliente llama `POST /api/v1/contracts/:contractId/entities/:entityTypeId/records/validate-unique` para los campos unique soportados; la escritura final sigue siendo autoritativa y puede rechazar colisiones entre dispositivos offline o cambios concurrentes.

Los tipos preventivos admitidos son los escalares escribibles que Core valida para unicidad; `RELATION`, `FILE` e `IMAGE` quedan excluidos. La normalizacion usa el mismo contrato que el submit: texto trimmeado, numeros/fechas/horas canonicos, `SELECT` por valor de opcion y `MULTISELECT` como arreglo normalizado. Valores vacios no participan; `0` y `false` son valores reales y se validan. Red/timeout y 5xx no bloquean el formulario como si fueran duplicado: se deja una advertencia o se difiere al submit autoritativo. 404/400/401/403 se tratan como errores de contrato/autorizacion segun el codigo real. Las respuestas tardias se atan a la generacion/valor validado y no deben sobrescribir un valor mas nuevo.
