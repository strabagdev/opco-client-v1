...
`REPORT` es consulta/presentacion: no crea, edita ni sincroniza registros locales. Soporta `TABLE`, `MATRIX` y `CURRENT_STATUS`. `TABLE`/`MATRIX` usan `timeFilter.mode = RANGE | MONTH`, `defaultPeriod = CURRENT_MONTH`, `allowChange`, y `valueDisplay[fieldId] = LABEL | INTERNAL_VALUE`. `CURRENT_STATUS` muestra una fila por registro relacionado con nombre, estado actual y fecha solo cuando `currentStatus.dateFieldId` esta configurado; no usa fechas de auditoria como reemplazo ni singulariza nombres por heuristica. El encabezado del registro relacionado usa primero el nombre del campo RELATION, luego un singular explicito de entidad si llega desde API, luego el nombre de entidad sin modificar y finalmente `Registro`. `INTERNAL_VALUE` se muestra en reportes en mayusculas solo como presentacion; no modifica datos, opciones ni API. `REPORT` no es `BOARD` ni `DASHBOARD`.
