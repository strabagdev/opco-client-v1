...
El indicador del encabezado combina punto y texto en un unico control accesible. Muestra `Al día` cuando esta online sin actividad, pendientes ni problemas; `Sincronizando`, `Comprobando disponibilidad` o `Restaurando sesión` durante esas actividades; `Cambios pendientes` cuando existe trabajo durable sin envio activo; `Sin conexión` offline; y `Requiere atención` ante error, conflicto o recuperación local. Conserva los colores, prioridades y animación existentes. `Al día` describe el estado global de conexión y trabajo pendiente, no garantiza que todas las cachés estén completas o recién actualizadas. Al pulsar el control abre directamente `Diagnostico > Sincronizacion`, mientras el botón general de Diagnóstico y todas sus pestañas permanecen disponibles. Las lecturas de PANEL y la preparación offline de AppViews no representan envío de cambios y no activan ese pulso; la preparación offline conserva su banner y detalle en `Diagnostico > PWA`.
